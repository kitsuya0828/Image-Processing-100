...
def max_pooling(img: np.ndarray, kernel_size: int = 8):
    H, W, C = img.shape
    ks = kernel_size
    ...
    for i in range(H // ks):
        for j in range(W // ks):
            for c in range(C):
                out[i*ks: (i+1)*ks, j*ks: (j+1)*ks, c] = np.max(img[i*ks: (i+1)*ks, j*ks: (j+1)*ks, c]).astype(int)
