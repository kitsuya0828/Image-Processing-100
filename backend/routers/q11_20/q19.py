import matplotlib.pyplot as plt
import numpy as np
import cv2
import datetime
from PIL import Image


def bgr2gray(img: np.ndarray):
    b = img[:, :, 0].copy()
    g = img[:, :, 1].copy()
    r = img[:, :, 2].copy()
    y = 0.2126 * r + 0.7152 * g + 0.0722 * b
    return y.astype(np.uint8)


def emboss_filter(img: np.ndarray, sigma: int = 3, kernel_size: int = 5):
    H, W = img.shape

    # ゼロ埋め
    pad = kernel_size // 2
    out = np.zeros((H + pad * 2, W + pad * 2), dtype=np.float32)
    out[pad: pad + H, pad: pad + W] = img.copy().astype(np.float32)

    tmp_out = out.copy()

    # カーネル
    kernel = np.zeros((kernel_size, kernel_size), dtype=np.float32)
    for y_index in range(kernel_size):
        for x_index in range(kernel_size):
            x = x_index - pad
            y = y_index - pad
            kernel[y_index, x_index] = (x**2 + y**2 - sigma**2) / (2 * np.pi * sigma**6) * np.exp(- (x**2 + y**2) / (2 * sigma**2))
    kernel /= kernel.sum()
    

    # フィルタリング
    for y in range(H):
        for x in range(W):
            out[pad + y, pad + x] = np.sum(kernel * tmp_out[y: y + kernel_size, x: x + kernel_size])

    out = np.clip(out, 0, 255)

    out = out[pad: pad + H, pad: pad + W].astype(np.uint8)

    return out


def solve(file_path: str, save_dir: str = "files/"):
    img = cv2.imread(file_path)

    img = bgr2gray(img)
    img_result = emboss_filter(img)

    dt_now = datetime.datetime.now()
    save_path = f"{dt_now.strftime('%Y%m%d%H%M%S')}.jpg"
    cv2.imwrite(save_dir + save_path, img_result)
    return {"path": save_path}


if __name__ == "__main__":
    sample_path = "../../files/sample/imori_noise.jpeg"
    save_dir = "../../files/"
    result_path = save_dir + solve(sample_path, save_dir)["path"]

    plt.figure(figsize=(12, 3))
    plt.subplot(1, 2, 1)
    plt.title('input')
    sample_image = Image.open(sample_path)
    sample_array = np.asarray(sample_image)
    plt.imshow(sample_array)

    plt.subplot(1, 2, 2)
    plt.title('output')
    result_image = Image.open(result_path)
    result_array = np.asarray(result_image)
    plt.imshow(result_array, cmap='gray')
    plt.show()
